...
| IE-014 | Provide an explicit portable private-document mode. | Pass; GUI-private is the default and disables preview, uses opaque private-root output and a removed-after-use worker input, redacts result paths/names, provides no visualization/archive, blocks Gradio access to the private output root, and removes its single reusable upload cache on GUI shutdown. |
